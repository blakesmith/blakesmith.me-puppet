define duplicity(
  ...
  file { "/root/backup/${name}-status":
    owner => "root",
    group => "root",
    mode => 700,
    content => template("duplicity/duplicity-status.erb"),
    require => File["/root/backup"]
  }
  
  file { "/root/backup/${name}-restore":
    owner => "root",
    group => "root",
    mode => 700,
    content => template("duplicity/duplicity-restore.erb"),
    require => File["/root/backup"]
  }

  file { "/root/backup/${name}-list":
    owner => "root",
    group => "root",
    mode => 700,
    content => template("duplicity/duplicity-list.erb"),
    require => File["/root/backup"]
  }
